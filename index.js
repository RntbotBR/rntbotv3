...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:meu criador✧⃟🔥🆁︎𝖊ⷷ𝖓ᷡ𝖆ⷶ𝖙ⷮ𝖔ᷳ✧͢⃟ᤢ☠︎︎͜͡🇧🇷\n' 
            + 'ORG:🔥☠︎︎𝗥⃟𝗡𝗧BOT☠︎︎͜͡🇧🇷🔥;\n' 
...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'Prontinho conectado com o Rnt-bot')
	})
	...
	const mdata = await client.groupMetadata(anu.jid)
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐎𝐩𝐚 @${num.split('@')[0]}\n𝐒𝐞𝐣𝐚 𝐁𝐞𝐦 𝐯𝐢𝐧𝐝𝐨 (𝐚) 𝐚𝐨 𝐠𝐫𝐮𝐩𝐨 *${mdata.subject}*\n\n𝐥𝐞𝐢𝐚 𝐚𝐬 𝐫𝐞𝐠𝐫𝐚𝐬 𝐝𝐨 𝐠𝐫𝐮𝐩𝐨 𝐧𝐚 𝐝𝐞𝐬𝐜𝐫𝐢𝐜̧𝐚̃𝐨 𝐩𝐚𝐫𝐚 𝐧𝐚𝐨 𝐬𝐞𝐫 𝐛𝐚𝐧𝐢𝐝𝐨❤️`
...
client.on('CB:action,,battery', json => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Estou fazendo aguarde um momento, se não der certo tente novamente⌛',
            ...
            const premium = ["558897966858@s.whatsapp.net","${setting.vip2}@s.whatsapp.net","${setting.vip3}@s.whatsapp.net","${setting.vip4}@s.whatsapp.net","${setting.vip5}@s.whatsapp.net","${setting.vip6}@s.whatsapp.net","${setting.vip7}@s.whatsapp.net","${setting.vip8}@s.whatsapp.net","${setting.vip9}@s.whatsapp.net","${setting.vip10}@s.whatsapp.net",]
...
case 'antifake':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐓𝐫𝐚𝐬𝐦𝐢𝐬𝐚̃𝐨 🔥☠︎︎𝗥⃟𝗡𝗧BOT☠︎︎͜͡🇧🇷🔥 ]\n\n${body.slice(4)}`)
						}
...
case 'aviso':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐀𝐯𝐢𝐬𝐨 🔥☠︎︎𝗥⃟𝗡𝗧BOT☠︎︎͜͡🇧🇷🔥 ]\n\n${body.slice(7)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
case 'divulgar':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐕𝐢𝐝𝐞𝐨 𝐧𝐨𝐯𝐨 𝐝𝐨 𝐑𝐍𝐓 ]\n\n${body.slice(10)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
...
case 'play':   
			    ...
			    buffer = await getBuffer(`https://api-exteam.herokuapp.com/api/card-spotify?titulo=${encodeURIComponent(anu.result.title)}&author=${encodeURIComponent(anu.result.source)}&album=RNT-BOT&capa=${anu.result.thumbnail}`)
				...
				foto = await getBuffer(`https://api-exteam.herokuapp.com/api/card-spotify?titulo=${encodeURIComponent(anu.result.title)}&author=${encodeURIComponent(anu.result.source)}&album=RNT-BOT&capa=${anu.result.thumbnail}`)
